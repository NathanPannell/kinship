...
export function PeopleScreen() {
  ...
  return (
    <>
      ...
      <section className={`people-bulk-panel${hasSelection ? " has-selection" : ""}`} aria-label="Bulk actions for selected people">
        <div className="people-bulk-heading"><strong className="people-bulk-count" aria-live="polite" aria-atomic="true">{selectedCount} {selectedCount === 1 ? "person" : "people"} selected</strong><button className="text-button" type="button" onClick={() => setSelectedIds(new Set())} disabled={!hasSelection || bulkBusy}>Clear selection</button></div>
        <div className="people-bulk-controls">
          <label className="people-bulk-field">Priority<select className="select" value={bulkPriority} onChange={(event) => setBulkPriority(event.target.value as Priority | "")} disabled={!hasSelection || bulkBusy}><option value="">Keep current</option><option value="high">High</option><option value="normal">Normal</option><option value="low">Low</option></select></label>
          <label className="people-bulk-field">Cadence in days<input className="input" type="number" min="1" max="3650" step="1" value={bulkCadence} onChange={(event) => setBulkCadence(event.target.value)} placeholder="Keep current" disabled={!hasSelection || bulkBusy} /></label>
          <button className="button button-primary" type="button" onClick={() => void applyBulkChanges()} disabled={!hasSelection || bulkBusy || (!bulkPriority && !bulkCadence)}>{bulkBusy ? "Working..." : "Apply to selected"}</button>
          <button className="button button-danger" type="button" onClick={() => void deleteSelected()} disabled={!hasSelection || bulkBusy}><Trash2 size={14} aria-hidden="true" />Delete selected</button>
        </div>
      </section>
      ...
      {error ? <div className="error-state"><div><RefreshCw size={20} aria-hidden="true" /><strong>Could not load your people</strong><p>{error}</p><button className="button button-secondary" type="button" onClick={() => { setLoading(true); void load(); }}>Try again</button></div></div> : loading ? <div className="loading-state"><div><RefreshCw size={20} className="spin" aria-hidden="true" /><strong>Loading your circle...</strong></div></div> : contacts.length === 0 ? <div className="empty-state"><div><Users size={24} aria-hidden="true" /><strong>{search || priority || overdue ? "No people match this view." : "Your circle is waiting."}</strong><p>{search || priority || overdue ? "Try clearing a filter to see more people." : "Bring in your LinkedIn connections or add someone you want to keep close."}</p><div className="empty-actions">{search || priority || overdue ? <button className="button button-secondary" type="button" onClick={() => { setSearch(""); setPriority(""); setOverdue(false); }}>Clear filters</button> : <><Link className="button button-accent" href="/onboarding">Import LinkedIn connections</Link><button className="button button-secondary" type="button" onClick={() => setCreating(true)}>Add a person</button></>}</div></div></div> : <>
