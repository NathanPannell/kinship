...
const contact = { type: "object", properties: {
  ...
  linkedin_url: { type: ["string", "null"] }, priority: { type: "string", enum: ["high", "normal", "low"] }, cadence_days: { type: "integer" },
...
export async function GET(request: Request) {
  const spec = {
    ...
    paths: {
      ...
      "/api/agent/contacts/{id}": { get: get("Get one contact and recent interaction history", { type: "object", properties: { contact, interactions: { type: "array", items: interaction } } }, [{ in: "path", name: "id", required: true, schema: { type: "string", format: "uuid" } }]) },
