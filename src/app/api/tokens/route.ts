...
export async function GET(request: NextRequest) {
  ...
  try {
    const tokens = await query<TokenRow>("SELECT id, name, token_prefix, created_at, revoked_at FROM api_tokens ORDER BY created_at DESC");
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [row] = await query<TokenRow>(
      "INSERT INTO api_tokens (name, token_hash, token_prefix) VALUES ($1, $2, $3) RETURNING id, name, token_prefix, created_at, revoked_at",
      [name, hash, `${token.slice(0, 12)}…`],
    );
