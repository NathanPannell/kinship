...
export async function GET(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const [photo] = await query<PhotoRow>(
      "SELECT encode(image_data, 'base64') AS image_base64, updated_at FROM contact_photos WHERE contact_id=$1",
      [id],
    );
...
export async function DELETE(request: NextRequest, context: Context) {
  ...
  try {
    const id = await contactIdFrom(context);
    const [contact] = await query<{ exists: boolean }>("SELECT EXISTS (SELECT 1 FROM contacts WHERE id=$1) AS exists", [id]);
    if (!contact?.exists) return privateJson({ error: "Person not found" }, 404);
    await query("DELETE FROM contact_photos WHERE contact_id=$1", [id]);
