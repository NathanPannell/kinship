...
describe.skipIf(!devUrl)("LinkedIn import against a development database", () => {
  if (devUrl) process.env.DATABASE_URL = devUrl;
  afterAll(async () => { await query("DELETE FROM contacts WHERE name=$1", [name]); });
  it("creates once, then fills missing fields without changing personal settings", async () => {
    ...
    const first = await commitImport(planImport([row], []));
    expect(first.created).toBe(1);
    const [saved] = await query<Contact>("SELECT * FROM contacts WHERE name=$1", [name]);
    await query("UPDATE contacts SET company=NULL, priority='high', cadence_days=21, notes='Keep this' WHERE id=$1", [saved.id]);
    const existing = await query<Contact>("SELECT * FROM contacts WHERE id=$1", [saved.id]);
    const second = await commitImport(planImport([row], existing));
    expect(second.updated).toBe(1);
    const [updated] = await query<Contact>("SELECT * FROM contacts WHERE id=$1", [saved.id]);
